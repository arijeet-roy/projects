...
//		String end = "AACCGGTA";
//		String data[] = new String[] {"AACCGGTA"};
	...
	private static int findMutationDistanceOptimized(String start, String end, String[] bank) {
		// TODO Auto-generated method stub
		...
		for(String word : bank) {
			...
			visited.add(word);
			minDistance = Math.min(minDistance, findDistance(word, end, visited, bank, 1));
			visited.remove(word);
		}
	...
	private static int findDistance(String s, String end, Set<String> visited, String[] data, int level) {
		...
		for (String word : data) {
			...
			visited.add(word);
			min = Math.min(min, findDistance(word, end, visited, data, level+1));
			visited.remove(word);
		}
		return min;
	}
